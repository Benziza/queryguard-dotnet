...
- Remediation is offered as *strategies to review*: eager loading, projection, batching, or a
  documented allowlist entry. QueryGuard never claims to have identified the fix.
...
A detector exists that can produce real semantic evidence of an N+1. For example, it could
correlate command execution with the materialization of a parent result set or with call-site
identity. It must also be validated against the false-positive fixture suite.
